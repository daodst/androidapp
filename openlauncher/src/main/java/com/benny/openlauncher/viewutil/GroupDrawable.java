...
public class GroupDrawable extends Drawable {
    ...
    public GroupDrawable(Context context, Item item, int iconSize) {
        ...
        for (int i = 0; i < 4 && i < item.getItems().size(); i++) {
            ...
            if (app == null) {
                ...
                if (temp != null && temp._type == Item.Type.STABLE) {
                    
                    _icons[i] = temp.getIcon();
                } else icons[i] = new ColorDrawable(Color.TRANSPARENT);
            } else {
    ...
    private void init(Drawable[] icons, int iconsCount, float size) {
        ...
        _paintInnerCircle.setAlpha(255);
        _paintInnerCircle.setAntiAlias(true);
        Bitmap bitmap = BitmapFactory.decodeResource(AppObject.get().getApplicationContext().getResources(), R.mipmap.group_bg);
        BitmapShader bitmapShader = new BitmapShader(bitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
        Matrix matrix = new Matrix();
        float scale = _iconSize / bitmap.getHeight();
        matrix.setScale(scale, scale);
        bitmapShader.setLocalMatrix(matrix);
        _paintInnerCircle.setShader(bitmapShader);
