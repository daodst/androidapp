...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String SQL_CREATE =
            "CREATE TABLE " + TABLE_HOME + " ("
                    + COLUMN_TIME + " VARCHAR(256) PRIMARY KEY,"
                    + COLUMN_TYPE + " VARCHAR,"
    ...
    public void createItem(Item item, int page, Definitions.ItemPosition itemPosition) {
        ...
        switch (item.getType()) {
            ...
            case SHORTCUT:
                ...
                break;
            case STABLE:
                Tool.saveIcon(_context, Tool.drawableToBitmap2(item.getIcon()), item.getId());
            ...
            case WIDGET:
                ...
                break;
            case CUSTOM_WIDGET:
                concat = item.getCustomWidgetType()
                        + Definitions.DELIMITER
                        + item.getSpanX()
                        + Definitions.DELIMITER
                        + item.getSpanY();
        ...
        try {
            _db.replace(TABLE_HOME, null, itemValues);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private Item getSelection(Cursor cursor) {
        ...
        switch (type) {
            ...
            case SHORTCUT: {
                item.setIntent(Tool.getIntentFromString(data));
                item.setIcon(Tool.getIcon(_context, item.getId()));
            ...
            case CUSTOM_WIDGET:
                dataSplit = data.split(Definitions.DELIMITER);
                item.setCustomWidgetType(Integer.parseInt(dataSplit[0]));
                item.setSpanX(Integer.parseInt(dataSplit[1]));
                item.setSpanY(Integer.parseInt(dataSplit[2]));
                break;
        }
