...
public final class Desktop extends ViewPager implements DesktopCallback {
    ...
    private final List<MyViewGroup> _pages = new ArrayList<>();
    ...
    @Deprecated
    private final int negativeOneScreen = 1;
    ...
    public static boolean handleOnDropOver(HomeActivity homeActivity, Item dropItem, Item item, View itemView, MyViewGroup parent, int page, ItemPosition itemPosition, DesktopCallback callback) {
        if (item != null) {
            if (dropItem != null) {
                ...
                if (type != null) {
                    switch (type) {
                        ...
                        case SHORTCUT:
                        case STABLE:
                            if (Type.APP.equals(dropItem._type) || Type.SHORTCUT.equals(dropItem._type) || Type.STABLE.equals(dropItem._type)) {
                            ...
                            break;
                        case GROUP:
                            if ((Item.Type.APP.equals(dropItem._type) || Type.SHORTCUT.equals(dropItem._type) || Type.STABLE.equals(dropItem._type)) && item.getGroupItems().size() < GroupPopupView.GroupDef._maxItem) {
                                parent.removeView(itemView);
                                dropItem._location = ItemPosition.Group;
    ...
    public static final class DesktopAdapter extends PagerAdapter {
        ...
        public DesktopAdapter(Desktop desktop) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (i == 0) _desktop.getPages().add(new CustomCellContainer(_desktop.getContext()));
                else _desktop.getPages().add(getItemLayout());
            }
            _desktop._desktopEditListener.onLoadComplete();
        }
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            MyViewGroup layout = _desktop.getPages().get(position);
            container.addView(((ViewGroup) layout));
            return layout;
        }
        ...
        private void enterDesktopEditMode() {
            ...
            for (MyViewGroup v : _desktop.getPages()) {
    ...
    public final List<MyViewGroup> getPages() {
        return _pages;
    }
    ...
    public final MyViewGroup getCurrentPage() {
    ...
    private void addItemsToPage() {
        ...
        if (desktopItems.size() > _pages.size()) {
            addPageEnd(false);
        }
        ...
        for (int pageCount = 0; pageCount < desktopItems.size(); pageCount++) {
            MyViewGroup group = _pages.get(pageCount);
            if (group instanceof CellContainer) {
    ...
    public final void addPageRight(boolean showGrid) {
        ...
        if (Setup.appSettings().getDesktopShowGrid()) {
            for (MyViewGroup cellContainer : _pages) {
    ...
    public final void addPageEnd(boolean showGrid) {
    ...
    public final void updateIconProjection(int x, int y) {
        ...
        switch (state) {
            ...
            case CurrentOccupied:
                ...
                for (MyViewGroup page : _pages) {
                    if (page instanceof CellContainer) {
                        page.clearCachedOutlineBitmap();
                    }
                }
                if (!type.equals(Type.WIDGET) && !type.equals(Type.CUSTOM_WIDGET) && (getCurrentPage().coordinateToChildView(_coordinate) instanceof AppItemView)) {
    ...
    public void revertLastItem() {
        if (_previousItemView != null) {
            if (_adapter.getCount() >= _previousPage && _previousPage > -1) {
                MyViewGroup cellContainer = _pages.get(_previousPage);
    ...
    public boolean addItemToPoint(@NonNull Item item, int x, int y) {
        ...
        x -= (item._spanX - 1) * getCurrentPage().getCellWidth() / 2f;
        y -= (item._spanY - 1) * getCurrentPage().getCellHeight() / 2f;
        item._location = ItemPosition.Desktop;
        if (x >= 0 && y >= 0) {
