...
data class VerificationBottomSheetViewMainState(
...
class VerificationBottomSheetMainViewModel @AssistedInject constructor(
        @Assisted initialState: VerificationBottomSheetViewMainState,
        private val session: Session,
        private val supportedVerificationMethodsProvider: SupportedVerificationMethodsProvider,
        private val stringProvider: StringProvider) :
        VectorViewModel<VerificationBottomSheetViewMainState, VerificationAction, VerificationBottomSheetViewEvents>(initialState),
        VerificationService.Listener {

    @AssistedFactory
    interface Factory : MavericksAssistedViewModelFactory<VerificationBottomSheetMainViewModel, VerificationBottomSheetViewMainState> {
        override fun create(initialState: VerificationBottomSheetViewMainState): VerificationBottomSheetMainViewModel
    }

    companion object : MavericksViewModelFactory<VerificationBottomSheetMainViewModel, VerificationBottomSheetViewMainState> by hiltMavericksViewModelFactory()
    ...
    init {
        ...
        setState {
            ...
            copy(
                    ...
                    verificationId = pr.transactionId ?: pr.localId,
                    selfVerificationMode = true
            )
        }
    }

    fun initStatus() = withState { state ->
        
        session.cryptoService().verificationService().addListener(this)

        val userItem = session.getUser(state.otherUserId)

        var autoReady = false
        val pr = if (state.selfVerificationMode) {
            

            session.cryptoService().verificationService().getExistingVerificationRequests(state.otherUserId)
            ...
            session.cryptoService().verificationService().getExistingVerificationRequest(state.otherUserId, state.verificationId)
        }

        val sasTx = (pr?.transactionId ?: state.verificationId)?.let {
            session.cryptoService().verificationService().getExistingTransaction(state.otherUserId, it) as? SasVerificationTransaction
        }

        val qrTx = (pr?.transactionId ?: state.verificationId)?.let {
            session.cryptoService().verificationService().getExistingTransaction(state.otherUserId, it) as? QrCodeVerificationTransaction
        }
        ...
        setState {
            copy(
                    ...
                    transactionId = pr?.transactionId ?: state.verificationId,
                    pendingRequest = if (pr != null) Success(pr) else Uninitialized,
                    isMe = state.otherUserId == session.myUserId,
    ...
    private fun cancelAllPendingVerifications(state: VerificationBottomSheetViewMainState) {
