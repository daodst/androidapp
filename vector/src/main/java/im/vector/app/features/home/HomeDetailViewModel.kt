        ...
        viewModelScope.launch(Dispatchers.Default) {
            val roomIds = session.getRoomSummaries(
                    roomSummaryQueryParams {
                        ...
                        roomCategoryFilter = if (state.currentTab is HomeTab.RoomList) {
                            if (state.currentTab.displayMode == RoomListDisplayMode.PEOPLE) {
                                RoomCategoryFilter.NOT_GROUP
                            } else {
                                RoomCategoryFilter.ONLY_GROUP
                            }
                        } else {
                            RoomCategoryFilter.ONLY_WITH_NOTIFICATIONS
                        }
                    }
            ).filter {
                it.notificationCount > 0 || it.atMsg.isNotEmpty()
            }.map {
                Timber.i("-----handleMarkAllRoomsRead-------${it.displayName}---------------")
                it.roomId
            }
                        ...
                        is RoomGroupingMethod.BySpace       -> {
                            ...
                            setState {
                                copy(
                                        ...
                                        notificationNormalCountPeople = notGroupRooms.totalCount + newroomsInvite,
                                        notificationNormalHighlightPeople = notGroupRooms.totalCount + newroomsInvite > 0,
                                        notificationGroupCountPeople = groupRooms.totalCount,
                                        notificationGroupHighlightPeople = groupRooms.totalCount > 0,
                                )
                            }
                        }
                        null                                -> Unit
