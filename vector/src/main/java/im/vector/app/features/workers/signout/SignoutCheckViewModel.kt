...
data class SignoutCheckViewState(
        ...
        val crossSigning: Boolean = false,
...
class SignoutCheckViewModel @AssistedInject constructor(
    ...
    sealed class Actions : VectorViewModelAction {
        ...
        object InitializeCrossSigning : Actions()
        object DelBaks : Actions()
    }
    ...
    init {
        ...
        setState {
            copy(
                    ...
                    crossSigning =xSigningKeyCanSign
            )
        }
    ...
    override fun handle(action: Actions) {
        when (action) {
            ...
            Actions.InitializeCrossSigning          -> {
    ...
    private fun delBaks() {
        val keysBackupService: KeysBackupService = session.cryptoService().keysBackupService()
        ...
        if (keysBackupService.currentBackupVersion != null) {
            keysBackupService.deleteBackup(keysBackupService.currentBackupVersion!!, object : MatrixCallback<Unit> {
                override fun onSuccess(data: Unit) {
                    Timber.i("======SignoutCheckViewModel============onSuccess==")
                }

                override fun onFailure(failure: Throwable) {
                    Timber.i("=====SignoutCheckViewModel===========onFailure====")
                }
            })
        }
    }

    private fun initializeCrossSigning() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                setState { copy(crossSigning = true) }
                awaitCallback<Unit> {
                ...
                val xSigningKeyCanSign = session.cryptoService().crossSigningService().canCrossSign()
                setState { copy(crossSigning = xSigningKeyCanSign) }
            } finally {
