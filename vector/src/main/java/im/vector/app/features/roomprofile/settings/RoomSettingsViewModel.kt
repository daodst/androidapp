...
class RoomSettingsViewModel @AssistedInject constructor(@Assisted initialState: RoomSettingsViewState,
    ...
    override fun handle(action: RoomSettingsAction) {
        when (action) {
            ...
            is RoomSettingsAction.Save                     -> saveSettings(action)
    ...
    private fun saveSettings(action: RoomSettingsAction.Save) = withState { state ->
        ...
        viewModelScope.launch {
            if (summary?.name != state.newName && summary?.isGroup == true) {
                updateName(action.activity, state.newName ?: "", operationList);
            } else {
                updateChain(operationList)
            }
        }
    }

    private suspend fun updateChain(operationList: MutableList<suspend () -> Unit>) {
    ...
    private fun updateName(activity: Activity, newName: String, operationList: MutableList<suspend () -> Unit>) {

        
        var walletPay: IWalletPay? = null
        activity.applicationContext?.takeAs<IApplication>()?.apply {
            walletPay = getDelegate(ApplicationDelegate.MOODLE_TYPE_WALLET)?.walletPay
        }

        walletPay?.changeDeviceGroupName(activity, session.myOriginUId, room.roomId, newName, object : TranslationListener {
            override fun onFail(errorInfo: String?) {
                _viewEvents.post(RoomSettingsViewEvents.Failure(RuntimeException(errorInfo)))
            }
