...
class SearchResultController @Inject constructor(
...
) : TypedEpoxyController<SearchViewState>(), Timeline.Listener {
    ...
    private fun buildSearchResultItems(data: SearchViewState): List<EpoxyModel<*>> {
        ...
        data.searchResult.forEach { eventAndSender ->
            ...
            val spannable = eventAndSender.body
                ...
                GenericHeaderItem_()
                    ...
                    .text(
                        dateFormatter.format(
                            ...
                            DateFormatKind.EDIT_HISTORY_HEADER
                        )
                    )
                    .let { result.add(it) }
            }
            ...
            SearchResultItem_()
                ...
                .formattedDate(
                    dateFormatter.format(
                        ...
                        DateFormatKind.MESSAGE_SIMPLE
                    )
                )
                .spannable(spannable.toEpoxyCharSequence())
                .sender(eventAndSender.sender
                    ?: eventAndSender.event.senderId?.let {
                        session.getRoomMember(
                            it,
                            data.roomId
                        )
                    }?.toMatrixItem()
                ...
                .threadSummaryFormatted(
                    displayableEventFormatter.formatThreadSummary(event.threadDetails?.threadSummaryLatestEvent)
                        .toString()
                )
