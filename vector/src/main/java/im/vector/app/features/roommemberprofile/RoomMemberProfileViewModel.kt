...
class RoomMemberProfileViewModel @AssistedInject constructor(
    ...
    init {
        ...
        viewModelScope.launch(Dispatchers.Main) {
            ...
            viewModelScope.launch {
                session.getRemark(initialState.userId)?.let {
                    setState { copy(remark = it) }
                }
            }
        }
    ...
    private fun handleOpenOrCreateDm(action: RoomMemberProfileAction.OpenOrCreateDm) {
        viewModelScope.launch {
            ...
            if (action.shouldSendFlowers) {
                session.getRoom(roomId)?.sendGifts()
            }
            if (roomId != initialState.roomId) {
                _viewEvents.post(RoomMemberProfileViewEvents.OpenRoom(roomId = roomId, isNew = true))
            } else {
    ...
    private fun handleShareRoomMemberProfile() {
     ...
     fun handleRemarkModifyed(remark: String) {
         viewModelScope.launch {
             session.updateRemark(initialState.userId, remark)
             setState {
                 copy(remark = remark)
             }
             _viewEvents.post(RoomMemberProfileViewEvents.SaveRemarkSuccess)
         }
    }
}
