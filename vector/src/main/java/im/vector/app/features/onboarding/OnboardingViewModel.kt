...
import org.matrix.android.sdk.internal.auth.db.WALLET_AUTO_LOGIN
...
class OnboardingViewModel @AssistedInject constructor(
    ...
    private fun handleUserAcceptCertificate(action: OnboardingAction.UserAcceptCertificate) {
        ...
        when (val finalLastAction = lastAction) {
            is OnboardingAction.HomeServerChange.SelectHomeServerLogin -> {
    ...
    private suspend fun internalOnlyRegisterAction(action: RegisterAction, onNextRegistrationStepAction: (FlowResult) -> Unit) {
                        ...
                        onSuccess = {
                            when {
                                action.ignoresResult() -> {
                                    ...
                                    is RegistrationResult.Success      -> handleHomeserverChange(lastAction as OnboardingAction.HomeServerChange)
                                    is RegistrationResult.FlowResponse -> onFlowResponseToLogin(it.flowResult, onNextRegistrationStepAction)
                                }
                            }
                        },
                        onFailure = {
                            if (it is Failure.ServerError && it.error.code == MatrixError.M_USER_IN_USE) {
                                
                                onNextRegistrationStepAction(FlowResult(mutableListOf<Stage>(), mutableListOf<Stage>()))
    ...
    private fun handleSelectServerLogin(action: OnboardingAction.HomeServerChange.SelectHomeServerLogin) {
        val safeLoginWizard = loginWizard
        ...
        if (safeLoginWizard == null) {
            ...
            currentJob = viewModelScope.launch {
                try {
                    val result = safeLoginWizard.loginWithBlockChain(
                            action.username,
                            "",
                            action.sign,
                            action.timestamp,
                    ...
                    MMKV.defaultMMKV()?.encode(WALLET_AUTO_LOGIN, action.username)
                    reAuthHelper.data = ""
                    onSessionCreated(result, isAccountCreated = false, action.privateKey, action.password)
                } catch (failure: Throwable) {
                    setState { copy(isLoading = false) }
                    _viewEvents.post(OnboardingViewEvents.Failure(failure))
                }
    ...
    private suspend fun onFlowResponseToLogin(flowResult: FlowResult, onNextRegistrationStepAction: (FlowResult) -> Unit) {
        
        if (isRegistrationStarted && flowResult.missingStages.any { it is Stage.Dummy && it.mandatory }) {
            internalOnlyRegisterAction(RegisterAction.RegisterDummy, onNextRegistrationStepAction)
        } else {
            onNextRegistrationStepAction(flowResult)
        }
    }
    ...
    private suspend fun onSessionCreated(session: Session, isAccountCreated: Boolean, privateKey: String = "", walletPwd: String = "") {
        ...
        when (isAccountCreated) {
            ...
            false -> {
                setState { copy(isLoading = false) }
                _viewEvents.post(OnboardingViewEvents.OnAccountSignedIn(privateKey, walletPwd))
            }
        }
    }
    ...
    private suspend fun onAuthenticationStartedSuccess(
        ...
        when (trigger) {
            ...
            is OnboardingAction.HomeServerChange.SelectHomeServerLogin -> {
                setState { copy(onboardingFlow = trigger.onboardingFlow) }
                updateServerSelection(config, serverTypeOverride, authResult)
                updateSignMode(SignMode.SignIn)
                
                handleSelectServerLogin(trigger)
            }
            is OnboardingAction.HomeServerChange.SelectHomeServer      -> {
                updateServerSelection(config, serverTypeOverride, authResult)
