...
class VectorCallActivity : VectorBaseActivity<ActivityCallBinding>(), CallControlsView.InteractionListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val stringExtra = intent.getStringExtra(EXTRA_MODE)
        incoming = intent.getStringExtra(EXTRA_MODE) !== OUTGOING_CREATED
        Timber.tag(loggerTag.value).v("incoming_ringing is ${incoming}")
        if (stringExtra == INCOMING_RINGING) {
    ...
    private fun renderFullScreenMode(state: VectorCallViewState) {
        ...
        views.voiceVideoCallRate.text =
                "Lv${state.userrank}${getString(R.string.voice_video_call_rate_tips1)} ${state.userLimit / 1024} KB/S \n${getString(R.string.voice_video_call_rate_tips2)}";
        views.voiceVideoCallRate.isVisible = true
    ...
    private fun renderPiPMode(state: VectorCallViewState) {
        val callState = state.callState.invoke()
        views.voiceVideoCallRate.isVisible = false
    ...
    private fun setupRenderersIfNeeded() {
        ...
        views.fullscreenRenderer.setEnableHardwareScaler(true )
    ...
    private fun handleViewEvents(event: VectorCallViewEvents?) {
        ...
        when (event) {
            ...
            is VectorCallViewEvents.ConnectionSelfTimeout             -> onErrorTimoutConnect2()
            else                                                      -> Unit
        }
    }
    ...
    private fun onErrorTimoutConnect2() {
        MaterialAlertDialogBuilder(this)
                .setTitle(R.string.call_failed_no_connection)
                .setMessage(R.string.call_error_user_not_responding)
