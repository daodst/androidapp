...
class VectorCallViewModel @AssistedInject constructor(
    ...
    private val callListener = object : WebRtcCall.Listener {
        ...
        override fun onStateUpdate(call: MxCall) {
            ...
            if (callState is CallState.Answering) {
            }
    ...
    private val callManagerListener = object : WebRtcCallManager.Listener {
        ...
        override fun onAudioDevicesChange() {
            ...
            if (currentSoundDevice == CallAudioManager.Device.Phone) {
                callManager.proximityManager.start()
            } else {
                callManager.proximityManager.stop()
            }
            setState {
                copy(
    ...
    private fun getUserRankByAddr() {

        viewModelScope.launch {
            val userLimit = session.getLevel(session.myOriginUId)
            setState {
                copy(
                        userrank = userLimit
                )
    ...
    private fun getUserLimitByAddr() {
        viewModelScope.launch {

            while (isActive) {
                val userLimit = session.getUserLimitByAddr(session.myOriginUId)
                setState {
                    copy(
                            userLimit = userLimit
                    )
