...
abstract class AbsMessageItem<H : AbsMessageItem.Holder> : AbsBaseMessageItem<H>() {
    ...
    override fun bind(holder: H) {
        super.bind(holder)
        val local = attributes.informationData.messageLayout.isLocal
        if (attributes.informationData.messageLayout.showAvatar && !local) {
            ...
            if (attributes.informationData.izServerNotice) {
        ...
        if (attributes.informationData.messageLayout.showDisplayName && !local) {
        ...
        if (local) {
            ...
            holder.avatarImageView.apply {
                isVisible = true
                setImageResource(R.drawable.cus_msg_notice_ico)
                setBackgroundResource(R.drawable.bg_cus_avatar_border)
                updateLayoutParams {
                    width = attributes.avatarSize
                    height = attributes.avatarSize
                }
            ...
            holder.memberNameView.apply {
                ...
                text = context.getString(R.string.system_msg_title)
                setTextColor(ContextCompat.getColor(context, R.color.color03B384))
            }
        }
    }
    ...
    fun Attributes.getMemberNameColor() = messageColorProvider.getMemberNameTextColor(informationData.matrixItem)
