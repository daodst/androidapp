...
abstract class CallTileTimelineItem : AbsBaseMessageItem<CallTileTimelineItem.Holder>() {
    ...
    private fun renderRejectedStatus(holder: Holder) {
        ...
        if (attributes.informationData.sentByMe) {
            ...
            if (attributes.callKind.isVoiceCall) {
                holder.statusView.setStatus(R.string.call_tile_no_answer_voice, R.drawable.ic_voice_call_declined)
            } else {
                holder.statusView.setStatus(R.string.call_tile_no_answer_video, R.drawable.ic_video_call_declined)
            }
        }
    }
    ...
    companion object {
        private var STUB_ID = R.id.messageCallStub
    }
