...
class BootstrapBottomSheet : VectorBaseBottomSheetDialogFragment<BottomSheetBootstrapBinding>() {
    ...
    override fun invalidate() = withState(viewModel) { state ->
        when (state.step) {
            is BootstrapStep.CheckingMigration           -> {
                views.bootstrapIcon.visibility = View.INVISIBLE
            ...
            is BootstrapStep.SaveRecoveryKey             -> {
                ...
                viewModel.handle(BootstrapActions.Completed)
            }
    ...
    companion object {
        ...
        fun show(fragmentManager: FragmentManager, mode: SetupMode, privateKey: String? = null): BootstrapBottomSheet {
            return BootstrapBottomSheet().apply {
                isCancelable = false
                setArguments(Args(setUpMode = mode, privateKey = privateKey))
            }.also {
