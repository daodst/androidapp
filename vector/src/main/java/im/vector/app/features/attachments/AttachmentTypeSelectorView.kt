...
import im.vector.app.core.utils.PERMISSIONS_FOR_TAKING_PHOTO_STORE
...
class AttachmentTypeSelectorView(context: Context,
    ...
    init {
        ...
        views.attachmentRedPacketButton.configure(Type.RED_PACKET)
        views.attachmentVoiceTransButton.configure(Type.VOICE_TRANS)
    ...
    fun setAttachmentVisibility(type: Type, isVisible: Boolean) {
        when (type) {
            ...
            Type.RED_PACKET -> views.attachmentRedPacketButton
            Type.VOICE_TRANS -> views.attachmentVoiceTransButton
        }.let {
            if (it == views.attachmentStickersButton || it == views.attachmentStickersButton) {
                it.isVisible = false
            } else {
    ...
    enum class Type(val permissions: List<String>, @StringRes val tooltipRes: Int) {
        CAMERA(PERMISSIONS_FOR_TAKING_PHOTO_STORE, R.string.tooltip_attachment_photo),
        GALLERY(PERMISSIONS_FOR_TAKING_PHOTO_STORE, R.string.tooltip_attachment_gallery),
        ...
        RED_PACKET(PERMISSIONS_EMPTY, R.string.tooltip_attachment_red_packet),
        VOICE_TRANS(PERMISSIONS_FOR_AUDIO_IP_CALL, R.string.attachment_voice_trans)
    }
}
