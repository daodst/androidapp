...
sealed class RoomDetailViewEvents : VectorViewEvents {
    ...
    data class Loading(val show: Boolean) : RoomDetailViewEvents()
    data class CallStatus(val hasUserInfo: Boolean, val isVedio: Boolean) : RoomDetailViewEvents()
    ...
    data class CheckPermission(val event:  TimelineEvent) : RoomDetailViewEvents()
}
