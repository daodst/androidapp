...
abstract class RoomSummaryItem : VectorEpoxyModel<RoomSummaryItem.Holder>() {
    ...
    @EpoxyAttribute var izGroup: Boolean = false

    
    @EpoxyAttribute var hasRedEnvelope: Boolean = false

    
    @EpoxyAttribute var izNotice: Boolean = false

    @EpoxyAttribute var izServerNoticeGroup: Boolean = false

    
    @EpoxyAttribute var izOwner: Boolean = false

    
    @EpoxyAttribute var izDvmGroup: Boolean = false

    
    @EpoxyAttribute var hasAtInfo: Boolean = false

    
    @EpoxyAttribute var dstWidth: Int = 0
    ...
    override fun bind(holder: Holder) {
        ...
        holder.roomListat.text = "@"
        holder.roomListat.isVisible = hasAtInfo && !izServerNoticeGroup

        
        holder.roomListBalance.isVisible = hasAtInfo

        
        holder.roomListDvm.isVisible = izDvmGroup

        
        holder.roomListOwner.isVisible = izOwner


        holder.roomListBalance.text = lastFormattedEvent.charSequence
        ...
        if (izNotice) {
            holder.lastEventView.text = ""
            holder.lastEventView.isVisible = false
            holder.roomListBalance.isVisible = true
        } else {
            holder.roomListBalance.isVisible = false
            holder.lastEventView.isVisible = true
        ...
        if (izServerNoticeGroup) {
            holder.avatarImageView.setImageResource(R.drawable.cus_server_ico)
            holder.titleView.text = holder.typingView.context.getString(R.string.server_notice_name)
        } else {
        ...
        holder.roomLastEventViewParent.isInvisible = holder.typingView.isVisible
    ...
    class Holder : VectorEpoxyHolder() {
        val roomListat by bind<TextView>(R.id.roomListat)
        val roomListBalance by bind<TextView>(R.id.roomListBalance)
        val roomListDvm by bind<ImageView>(R.id.roomListDvm)
        val roomListOwner by bind<ImageView>(R.id.roomListOwner)
        val roomLastEventViewParent by bind<LinearLayout>(R.id.roomLastEventViewParent)
