...
class CreateRoomController @Inject constructor(
        ...
        private val roomNameErrorFormatter: RoomNameErrorFormatter
    ...
    private fun buildForm(viewState: CreateRoomViewState, enableFormElement: Boolean) {
        ...
        formEditTextItem {
            ...
            errorMessage(
                    host.roomNameErrorFormatter.format(
                            (((viewState.asyncCreateRoomRequest as? Fail)?.error) as? CreateRoomFailure.NameError)?.nameError
                    )
            )
            onTextChange { text ->
