...
class TimelineEventController @Inject constructor(private val dateFormatter: VectorDateFormatter,
    ...
    interface PosClickCallback {
        fun onPosClicked(type: String, param: String)
    }
    ...
    interface CusAwardCallback {
        fun onBalanceClicked(targetEvent: TimelineEvent)
    }

    interface CusDposCallback {
        fun onDposToClicked()
        fun onDposMethodClicked()
    }

    interface CusLordCallback {
        fun onLordInvitateClicked()
        fun onLordManageClicked()
    }

    interface IChartCallback {
        fun posClickListener(is3Day: Boolean)
        fun activeClickListener(is3Day: Boolean)
        fun lGClickListener(is3Day: Boolean)
    }

    interface CusClusterCallback {
        fun onCusClusterClicked()
    }
        ...
        (0 until modelCache.size).forEach { position ->
            ...
            if (modelCache[position] == null || modelCache[position]?.isCacheable(partialState) == false || reactionListFactory.needsRebuild(event)) {
                ...
                val params = TimelineItemFactoryParams(
                        izGroup = partialState.roomSummary?.isGroup ?: false,
                        izOwner = partialState.roomSummary?.owner == session.myOriginUId,
                        event = event,
                        prevEvent = prevEvent,
            ...
            modelCache[position] =
                    itemCachedData.enrichWithModels(event, nextEvent, position, receiptsByEvent)
        }
    ...
    private fun CacheItemData.enrichWithModels(event: TimelineEvent,
        ...
        return copy(
                ...
                mergedHeaderModel = if (izServerNotice) null else mergedHeaderModel
        )
    }
    ...
    private data class CacheItemData(
        ...
        override fun toString(): String {
            return "CacheItemData(localId=$localId, eventId=$eventId, readReceiptsItem=$readReceiptsItem, eventModel=$eventModel, mergedHeaderModel=$mergedHeaderModel, formattedDayModel=$formattedDayModel, isCacheable=$isCacheable)"
        }
    }
}
