...
import com.wallet.ctc.util.AllUtils.getAddressByUid
...
open class VectorApplication :
    ...
    fun onCreateInite() {
        ...
        if (!sessionImported) {
            ...
            doNotShowDisclaimerDialog(appContext)
        }
    ...
    fun callNotify() {
        ...
        val hasActiveCall = knownCalls.isNotEmpty()
        handler.post {
            setCallIsVisible(hasActiveCall)
        }

    }

    fun hasActiveCall(): Boolean {
        return callManager.getCalls().isNotEmpty()
    }

    open fun setCallIsVisible(isVisible: Boolean) {
    }
    ...
    fun isLogin(address: String): Boolean {
        ...
        return authenticationService.isLogin(address)
    }
