...
abstract class VectorBaseFragmentHost<VB : ViewBinding> : Fragment(), MavericksView {
    ...
    protected val vectorBaseActivity: AppCompatActivity by lazy {
   ...
   open var hasSavedInstanceState = false

    @CallSuper
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        hasSavedInstanceState = null != savedInstanceState
        if (getMenuRes() != -1) {
    ...
    protected fun showErrorInSnackbar(throwable: Throwable) {
        Toast.makeText(vectorBaseActivity, errorFormatter.toHumanReadable(throwable), Toast.LENGTH_SHORT).show()
    }

    var toolbar: ToolbarConfig? = null

    
    fun setupToolbar(toolbar: MaterialToolbar) = ToolbarConfig(vectorBaseActivity, toolbar).also {
