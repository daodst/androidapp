...
internal class DefaultRoomService @Inject constructor(
    ...
    override fun getAllRoom(queryParams: RoomSummaryQueryParams, sortOrder: RoomSortOrder): LiveData<List<RoomSummary>> {
        return roomSummaryDataSource.getAllRoomLive(queryParams, sortOrder)
    }
    ...
    override fun getLikeRoomMember(userId: String, roomId: String): RoomMemberSummary? {
        val roomMemberEntity = monarchy.fetchCopied {
            RoomMemberHelper(it, roomId).getLastLikeRoomMember(userId)
        }
        return roomMemberEntity?.asDomain()
    }
