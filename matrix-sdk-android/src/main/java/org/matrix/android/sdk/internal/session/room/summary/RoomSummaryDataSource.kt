...
internal class RoomSummaryDataSource @Inject constructor(
    ...
    fun setRoomSummaryGroupStatus(roomId: String, isGroup: Boolean) {
        taskExecutor.executorScope.asyncTransaction(monarchy) { realm ->
            RoomSummaryEntity.where(realm, roomId).findFirst()?.isGroup = isGroup
        }
    }

    fun setRoomSummaryDvmStatus(roomId: String, isDvmGroup: Boolean) {
        taskExecutor.executorScope.asyncTransaction(monarchy) { realm ->
            RoomSummaryEntity.where(realm, roomId).findFirst()?.isDvmGroup = isDvmGroup
        }
    }

    fun setRoomSummaryLevel(roomId: String, level: Int) {
        taskExecutor.executorScope.asyncTransaction(monarchy) { realm ->
            RoomSummaryEntity.where(realm, roomId).findFirst()?.level = level
        }
    }

    fun setRoomSummaryUpgradeAnimShow(roomId: String, show: Boolean) {
        taskExecutor.executorScope.asyncTransaction(monarchy) { realm ->
            RoomSummaryEntity.where(realm, roomId).findFirst()?.izUpgrade = show
        }
    }

    fun setRoomSummaryOwner(roomId: String, owner: String) {
        taskExecutor.executorScope.asyncTransaction(monarchy) { realm ->
            RoomSummaryEntity.where(realm, roomId).findFirst()?.owner = owner
        }
    }
    ...
    fun delAtMsg(roomId: String, eventId: String) {
        taskExecutor.executorScope.asyncTransaction(monarchy) { realm ->
            RoomSummaryEntity.where(realm, roomId).findFirst()?.let {
                val timelineEvent = it.atMsg.filter { it.eventId == eventId } ?: return@let
                it.atMsg.removeAll(timelineEvent)
            }
        }
    }

    fun clearAtMsg(roomId: String) {
        taskExecutor.executorScope.asyncTransaction(monarchy) { realm ->
            RoomSummaryEntity.where(realm, roomId).findFirst()?.atMsg?.clear()
        }
    }
    ...
    fun getAllRoomLive(queryParams: RoomSummaryQueryParams, sortOrder: RoomSortOrder): LiveData<List<RoomSummary>> {

        if (false) {
            
            monarchy.doWithRealm {
                roomSummariesQuery(it, queryParams).process(sortOrder).findAll().map {
                    roomSummaryMapper.map(it)
                }
            }
        }
        ...
        return Transformations.map(liveRooms) {
            it.realmResults.where().findAll().map {
                roomSummaryMapper.map(it)
            }
        }
    }
    ...
    fun getNotificationCountForRooms(queryParams: RoomSummaryQueryParams): RoomAggregateNotificationCount {
        ...
        monarchy.doWithRealm { realm ->
            ...
            val sumOf = roomSummariesQuery.findAll().sumOf {
                it?.atMsg?.size ?: 0
            }
            val notifCount = roomSummariesQuery.sum(RoomSummaryEntityFields.NOTIFICATION_COUNT).toInt() + sumOf
    ...
    private fun roomSummariesQuery(realm: Realm, queryParams: RoomSummaryQueryParams): RealmQuery<RoomSummaryEntity> {
        ...
        queryParams.roomCategoryFilter?.let {
            when (it) {
                ...
                RoomCategoryFilter.ONLY_GROUP              -> query.equalTo(RoomSummaryEntityFields.IS_GROUP, true)
                RoomCategoryFilter.NOT_GROUP               -> {
                    query.equalTo(RoomSummaryEntityFields.IS_GROUP, false)
                }
