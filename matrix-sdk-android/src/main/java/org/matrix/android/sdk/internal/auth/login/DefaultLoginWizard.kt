...
internal class DefaultLoginWizard(
    ...
    override suspend fun loginWithBlockChain(login: String, password: String, sign: String, timestamp: String, pubkey: String, initialDeviceName: String, deviceId: String?,  chat_pub_key: String,  chat_sign: String): Session {
        val loginParams = WalletLoginParams.userIdentifier(
                user = login,
                password = password,
                sign = sign,
                timestamp = timestamp,
        ...
        val credentials = executeRequest(null) {
            authAPI.loginWithBlockChain(loginParams)
        }

        return sessionCreator.createSession(credentials, pendingSessionData.homeServerConnectionConfig)
    }

    override suspend fun loginWithJwt(userId: String,
                               jwt: String,
                               initialDeviceName: String,
                               deviceId: String?): Session {
        val loginParams = JwtLoginParams.userIdentifier(
                user = userId,
                jwt = jwt,
                deviceDisplayName = initialDeviceName,
                deviceId = deviceId
        ...
        val credentials = executeRequest(null) {
            authAPI.loginWithJwt(loginParams)
        }
