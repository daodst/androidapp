...
internal class LoadTimelineStrategy(
    ...
    fun onStart() {
        Timber.tag("jues_param").v("onStart is :onStart")
        ...
        chunkEntity = getChunkEntity(realm).also {
            Timber.tag("jues_param").v("findAllSnapshot chunkEntity.size(): ${chunkEntity?.size}")
            it.addChangeListener(chunkEntityListener)
            timelineChunk = it.createTimelineChunk()
            Timber.tag("jues_param").v("findAllSnapshot timelineChunk2 == null: ${timelineChunk == null}")
        }
    }
    ...
    fun findAllSnapshot(listener: RealmChangeListener<MutableList<TimelineEvent>>){
        Timber.tag("jues_param").v("findAllSnapshot chunkEntity == null: ${chunkEntity == null}")
        Timber.tag("jues_param").v("findAllSnapshot timelineChunk == null: ${timelineChunk == null}")
        timelineChunk?.loadFromStorageForAll(0, Timeline.Direction.BACKWARDS,listener)
    }
    ...
    private fun getChunkEntity(realm: Realm): RealmResults<ChunkEntity> {
        Timber.tag("jues_param").v("findAllSnapshot Mode: $mode")
        return when (mode) {
            is Mode.Live      -> {
    ...
    private fun RealmResults<ChunkEntity>.createTimelineChunk(): TimelineChunk? {
        return firstOrNull()?.let {
            ...
            Timber.tag("jues_param").v("findAllSnapshot timelineChunk3 == null: ${chunk == null}")
            Timber.tag("jues_param").v("findAllSnapshot chunk: ${chunk.toString()}")
            return chunk
        }
    }
}
