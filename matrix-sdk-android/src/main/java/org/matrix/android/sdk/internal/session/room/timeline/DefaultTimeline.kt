...
internal class DefaultTimeline(private val roomId: String,
    ...
    override fun start(rootThreadEventId: String?, isSearch: Boolean) {
        timelineScope.launch {
    ...
    private suspend fun loadMore(count: Int, direction: Timeline.Direction, fetchOnServerIfNeeded: Boolean): Boolean {
        ...
        if (currentState.failCount > 3) {
            Timber.e("$baseLogMessage : loadmore fail")
            return false
        }
        ...
        if (loadMoreResult == LoadMoreResult.REACHED_END) {
            updateState(direction) {
                it.copy(loading = false, hasMoreToLoad = false, failCount = 0)
            }
        } else if (loadMoreResult == LoadMoreResult.FAILURE) {
            updateState(direction) {
                val count = it.failCount + 1;
                it.copy(loading = false, hasMoreToLoad = true, failCount = count)
            }
        } else if(loadMoreResult == LoadMoreResult.SUCCESS) {
            updateState(direction) {
                it.copy(loading = false, hasMoreToLoad = true, failCount = 0)
            }
        }
    ...
    private suspend fun openAround(eventId: String?, rootThreadEventId: String?) = withContext(timelineDispatcher) {
        ...
        Timber.tag("jues_param").v("strategy :${strategy.toString()}")
        Timber.tag("jues_param").v("openAround eventId:$eventId --- rootThreadEventId:$rootThreadEventId")
    ...
    private fun onLimitedTimeline() {
        Timber.v("sky  onLimitedTimeline")
        timelineScope.launch {
            initPaginationStates(null)
    ...
    private suspend fun findAllSnapshot() {
        val listener = RealmChangeListener<MutableList<TimelineEvent>> { it ->
            val list = it.orEmpty()
            Timber.v("Post snapshot of ${list.size} events")
            timelineScope.launch(coroutineDispatchers.main) {
                listeners.forEach {
                    tryOrNull { it.onFindAll(list) }
                }
            }
        }
        strategy.findAllSnapshot(listener)
