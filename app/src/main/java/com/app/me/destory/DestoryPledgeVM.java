...
public class DestoryPledgeVM extends BaseViewModel {
    private static final String TAG = "DestoryPledgeVM";
    ...
    public DestoryPledgeVM(@NonNull Application application) {
        super(application);
    ...
    public void getEvmosChatUnPledgeAvailable(String address) {
        showLoadingDialog("");
        mDisposable.add(mRpcApi.getEvmosChatUnPledgeAvailable(address).observeOn(AndroidSchedulers.mainThread()).subscribeWith(new NextSubscriber<EvmosChatUnPledgeAvailable>() {
            @Override
    ...
    public void getHashPldgeNum(String address, String name) {
        showLoadingDialog("");
        mDisposable.add(mRpcApi.getEvmosOneBalance(address, name).observeOn(AndroidSchedulers.mainThread()).subscribeWith(new NextSubscriber<EvmosOneBalanceBean>() {
            @Override
    ...
    public void getConfig(String address, String phoneStartNum, String key, String langStr) {
        showLoadingDialog("");


        String lang = "";
        if (TextUtils.equals(langStr, LanguageUtil.TYPE_LAGUAGE_TAIWAN_CHINESE) || TextUtils.equals(lang, LanguageUtil.TYPE_LAGUAGE_SIMPLE_CHINESE)) {
        ...
        Observable.zip(
                        ...
                        mRpcApi.getDaodstTips(key, lang),

                        (chatInfoBean, evmosTotalPledgeBean, evmosGatewayBean, evmosPledgeParamsBean, toBurn, ratio, tipsBean) -> {
                            boolean isSuccess = chatInfoBean.isSuccess() && evmosTotalPledgeBean.isSuccess()
                                    && evmosGatewayBean.isSuccess() && evmosPledgeParamsBean.isSuccess() && toBurn.isSuccess() && ratio.isSuccess() && null != evmosGatewayBean.data && tipsBean.isSuccess();
                            if (!isSuccess) {
                                String errorInfo = getErrorInfo(phoneStartNum, chatInfoBean, evmosTotalPledgeBean, evmosGatewayBean, evmosPledgeParamsBean, toBurn, ratio, tipsBean);
                                throw new Exception(errorInfo);
                            }
                            
                            return zipConfig(chatInfoBean, evmosTotalPledgeBean, evmosGatewayBean, evmosPledgeParamsBean, toBurn, ratio, tipsBean);
    ...
    private EvmosPledgeConfigBean zipConfig(EvmosChatInfoBean chatInfoBean, EvmosTotalPledgeBean evmosTotalPledgeBean,
                                            ...
                                            EvmosChatToBurn toBurn, EvmosChatBurnRatio ratio, DaodstTipsBean tipsBean) {
        EvmosPledgeConfigBean config = new EvmosPledgeConfigBean();

        
        if (null != tipsBean.data) {
            
            config.hash_pledge = TextUtils.isEmpty(tipsBean.data.hash_pledge) ? "" : tipsBean.data.hash_pledge;
            config.dst_burn = TextUtils.isEmpty(tipsBean.data.dst_burn) ? "" : tipsBean.data.dst_burn;
            try {
                config.undelegate_level = Integer.parseInt(tipsBean.data.undelegate_level);
            } catch (NumberFormatException e) {
                config.undelegate_level = -1;
            }
        } else {
            config.undelegate_level = -1;
        }
        ...
        config.pledge_hash_get = AllUtils.getTenDecimalValue(ratio.data.pledge_hash_get, 18, 4);
        config.burn_address = ratio.data.burn_address;
        ...
        config.min_burn_amount = getTenDecimalValue(evmosPledgeParamsBean.data.min_burn_amount.amount);
        ...
        config.minPledgeNumShow = getTenDecimalValue(setMinBigNum);
    ...
    private String getErrorInfo(String phoneStartNum, EvmosChatInfoBean chatInfoBean, EvmosTotalPledgeBean evmosTotalPledgeBean,
                                EvmosGatewayBean evmosGatewayBean, EvmosChatParamsBean evmosPledgeParamsBean, EvmosChatToBurn toBurn, EvmosChatBurnRatio ratio, DaodstTipsBean tipsBean) {
        String errorInfo = "";

        if (null == evmosGatewayBean.data) {
            errorInfo = String.format(getApplication().getString(com.app.R.string.phone_start_error), phoneStartNum);
        ...
        } else if (!TextUtils.isEmpty(tipsBean.getInfo())) {
            errorInfo = tipsBean.getInfo();
        }
        return errorInfo;
    }
    ...
    public void getWithdrawGas(String address, EvmosPledgeConfigBean config) {
        showLoadingDialog("");
        mDisposable.add(mRpcApi.getEvmosWithdrawGas(address, config.pledgeAddress)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new NextSubscriber<EvmosSeqGasBean>() {
                    @Override
                    public void dealData(EvmosSeqGasBean value) {
    ...
    public void getUnPledgeGas(String address, String nodeAddr, String num, EvmosPledgeConfigBean config) {
        ...
        mDisposable.add(mRpcApi.getEvmosUnPledgeGas(address, nodeAddr, config.tokenName, amountBigNum)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new NextSubscriber<EvmosSeqGasBean>() {
                    @Override
                    public void dealData(EvmosSeqGasBean value) {
    ...
    public void doPledggeRegistGas(boolean reg, String address, String num, String phonePrex, EvmosPledgeConfigBean config, WalletEntity wallet,
                                   String pwd, String delegatorAddr) {

        showLoadingDialog("");
        mDisposable.add(getEvmosAccountGas(!config.isHasRegisted && reg, address, config.pledgeAddress, config.tokenNameDestory, phonePrex, num, delegatorAddr)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new NextSubscriber<EvmosSeqGasBean>() {
                    @Override
                    public void dealData(EvmosSeqGasBean value) {
    ...
    void doPledggeRegist(EvmosSeqGasBean bean, String address, String num, String phonePrex, EvmosPledgeConfigBean config, WalletEntity wallet,
                         String pwd, String delegatorAddr) {
        showLoadingDialog("");
        registeOrPledgeSign(!config.isHasRegisted && bean.reg, address, num, config.pledgeAddress, bean.seqAccount, bean.gas.getGasAmount(),
    ...
    private Observable<EvmosSignResult> registeOrPledgeSign(boolean isRegsite, String address, String number, String nodeAddress,
        ...
        return Observable.create((ObservableOnSubscribe<EvmosSignResult>) emitter -> {
            ...
            if (isRegsite) {
                mType = TYPE_REGISTE;
                signByte = ChatSdk.signResgister(wallet.getChatAddress(), nodeAddress, amount, coinName, phonePrefix);
            } else {
                mType = TYPE_PLEDGE;
