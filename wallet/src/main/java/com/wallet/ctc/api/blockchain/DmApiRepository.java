...
public class DmApiRepository extends HttpDataRepositoryBase {
    private static DmApiRepository mDataRepository = null;
    private final String TAG = "BlockApiRepository";

    private DmApiRepository() {
    }

    public static DmApiRepository getInstance() {
        if (null == mDataRepository) {
            synchronized (DmApiRepository.class) {
                if (null == mDataRepository) {
                    mDataRepository = new DmApiRepository();
                }
            }
        }
    ...
    protected Retrofit useRetrofit() {
        return  HttpMethods.getInstance().getDmRetrofit();
    }
