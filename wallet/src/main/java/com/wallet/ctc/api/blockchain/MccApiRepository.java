...
public class MccApiRepository extends HttpDataRepositoryBase {
    private static MccApiRepository mDataRepository = null;
    private final String TAG = "BlockApiRepository";

    private MccApiRepository() {
    }

    public static MccApiRepository getInstance() {
        if (null == mDataRepository) {
            synchronized (MccApiRepository.class) {
                if (null == mDataRepository) {
                    mDataRepository = new MccApiRepository();
                }
            }
        }
    ...
    protected Retrofit useRetrofit() {
        return  HttpMethods.getInstance().getMccRetrofit();
    }
