...
public class AddWalletTypeAdapter extends BaseAdapter {
    private List<WalletLogoBean> list = new ArrayList<WalletLogoBean>();
    private Context context;

    public AddWalletTypeAdapter(Context context) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(mEntity.getChoose()==1){
            holder.choosedImg.setImageResource(R.mipmap.setting_wallet_def_choosed);
        }else {
            holder.choosedImg.setImageResource(R.mipmap.setting_wallet_def_choose);
        }
        if(mEntity.getIsdef()==1){
            holder.choosedImg.setVisibility(View.INVISIBLE);
        }else {
