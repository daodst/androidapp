...
public class TransferFilActivity extends BaseActivity {
    ...
    public void initUiAndListener() {
     ...
     gasCount=new BigDecimal(limitCount+"");
        ...
        walletTransctionUtil.setFiltransctionListen(new WalletTransctionUtil.FilTransctionListen() {
            @Override
            ...
            @Override
            public void showGasEstimateFeeCap(String defGasprice) {
              if(mLoadingDialog!=null)  mLoadingDialog.dismiss();
              gasprice=new BigDecimal(defGasprice);
           getprice(gasprice);
                data.setKuanggong(feiyongStr);
                data.setGasprice(defGasprice);
                data.setFee(feiyongStr.replace(mustCoin, ""));
                walletTransctionUtil.DoTransction(data,true);
            }

            @Override
            public void showGasmsg(FilGasMsgBean.ResultBean gasMsgBean) {
                if(mLoadingDialog!=null)  mLoadingDialog.dismiss();
                gasCount=new BigDecimal(gasMsgBean.getGasLimit()+"");
                data.setGascount(gasCount.intValue());
                data.setGasFeeCap(gasMsgBean.getGasFeeCap());
                data.setGasPremium(gasMsgBean.getGasPremium());
                walletTransctionUtil.getFilDefPrice(data);
            }
    ...
    public void onViewClicked(View view) {
        ...
        if (i == R.id.img_back) {
            ...
            if (TextUtils.isEmpty(toAddress) || !AnyAddress.isValid(toAddress, CoinType.FILECOIN)) {
            ...
            walletTransctionUtil.getGasEstimateMessageGas(data);
    ...
    private void getprice(BigDecimal gasprice) {
        ...
        Log.v("fsefsfsefs",gasprice.toPlainString()+"---"+gasCount.toPlainString()+"----"+feiyongStr);
    }
}
